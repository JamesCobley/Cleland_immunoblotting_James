...
for i, row in enumerate(triangle):
    for j, value in enumerate(row):
        ax_triangle.text(j - i / 2, -i, str(value), ha='center', va='center', fontsize=8, weight='bold')
...
for (row, col), cell in table.get_celld().items():
    ...
    cell.set_fontsize(10)  # Ensure all text is bold
ax_table.set_title("(B) Table of k-Space Contributions", fontsize=14, weight='bold')
...
plt.tight_layout()
file_path = "enhanced_combined_figure_with_bold_table.png"
